...
class ModuleOrderView(CsrfExemptMixin, JsonRequestResponseMixin, View):
    def post(self, request):
        for obj_id, order in self.request_json.items():
            Module.objects.filter(id=obj_id, course__owner=request.user).update(order=order)
...
class ContentOrderView(CsrfExemptMixin, JsonRequestResponseMixin, View):
    def post(self, request):
        for obj_id, order in self.request_json.items():
            Content.objects.filter(id=obj_id, module__course__owner=request.user).update(order=order)
        return self.render_json_response({'saved': 'OK'})
